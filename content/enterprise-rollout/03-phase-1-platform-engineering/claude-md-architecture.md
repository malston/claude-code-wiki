...
This is worse than a space problem. A [benchmark study of CLAUDE.md effectiveness](https://techloom.it/blog/claudemd-benchmark-results.html) found that adding generic coding instructions consistently reduced quality scores compared to no CLAUDE.md at all. The mechanism: generic instructions create an **adherence penalty** -- they don't make Claude try harder at things it already does, but they create a scorecard that can only subtract points. Even a handful of universal coding advice bullet points ("write clean code", "handle edge cases") made output worse.
